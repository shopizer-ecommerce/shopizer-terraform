...
Argo CD platform observability applications are generated by:

```text
k8s/argocd/applicationsets/observability.yaml
```

The ApplicationSet deploys these releases into the `monitoring` namespace:
...
The values source is:

```text
k8s/platform/values/local/<values-file>.yaml
```

The observability ApplicationSet reads the `shopizer-terraform` repository from
the `platform4.0` branch. That branch must contain the platform charts and values
before Argo CD can sync the applications.
